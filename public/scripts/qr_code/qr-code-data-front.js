...
async function activitiesTabClicked(){
	...
	const activityBrondon = await imAPIGet(`activities/${searchQuery}`);
	const respData = { activityId: activityBrondon.data.id, brondon: activityBrondon.data.brondons[0] }
	doConfetti();
	
	const tableBody = document.getElementById('reportTableBody');
	tableBody.innerHTML = ''; // Clear existing rows
	setupActivityEntry(tableBody, respData);
}
